...
def sample_action(Q, s, env, epsilon):
    ...
    return env.action_space.sample() if random.random() < epsilon else np.argmax(Q[s])
...
def q_learning(Q, env, epsilon, alpha, gamma, epis):
    ...
    for i in range(epis):
        ...
        while True:
            ...
            action = sample_action(Q, ob, env, epsilon)

            # TODO: Get new state & reward from environment
            new_ob, reward, terminated, truncated, _ = env.step(action)
            ep_reward += reward

            # TODO: Update Q-Table with new knowledge
            Q = q_update(Q, ob, action, reward, new_ob, alpha, gamma)
        ...
        if (i + 1) % 100 == 0:
            ...
            print(
                f"Episode {i + 1}/{epis} | Average Reward (last 100 episodes): {avg_reward:.2f}"
            )
...
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                action = torch.argmax(self.policy_net(state)).view(1, 1).to(self.device)
        else:
            action = torch.tensor(
    ...
    def optimize_model(self):
        if self.get_values() is not None:
            state_action_values, reward_batch, non_final_mask, non_final_next_states = (
                self.get_values()
            )
        else:
            return
...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.softmax(self.fc2(F.relu(self.fc1(x))))
...
def calculate_discounted_returns(episode_rewards, gamma):
    ...
    for t in range(T):
        discounted_return = discounted_return * gamma + episode_rewards[T - 1 - t]
